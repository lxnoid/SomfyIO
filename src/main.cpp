...
#ifdef ESP32
#define PIN_CH1 32 // 200Hz pulses while all channels lit, low active while channel 1 selected
#define PIN_CH 33
#define PIN_UP 25
#define PIN_MY 26
#define PIN_DWN 27
...
void setup()
{
  ...
  if (cfile)
  {
    ...
    Serial.println("...");
    DynamicJsonDocument cjson(config_data.length() + 13); //currently I don't understand the change as it worked with a real setup without the magic number?
    DeserializationError err = deserializeJson(cjson, config_data);
